<template>
  <div id="ux-timer"
       class="align-center cursor-pointer no-select"
       @click.stop="activeFn" v-text="timerText||btnText"></div>
</template>
...
<script>
  export default {
    name: 'uxTimer',
    props: {
      value: {type: Boolean, default: false}, // 是否激活
    ...
    computed: {
      btnText() { // 要展示的props文案
        const {focus, template, blur, type} = this
      ...
      timerFn() {
        ...
        return this.timer = setInterval(() => {
          if (time > 0) {
            ...
            [this.type, this.timerText] = [1, ''] // 完毕
            return this.$emit('end', this.type) || this.$emit('input', false) || clearInterval(this.timer)
          }
        }, 1000)
      },
